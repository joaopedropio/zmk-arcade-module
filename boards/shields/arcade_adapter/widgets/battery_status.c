...
#ifdef CONFIG_ARCADE_USE_BATTERY_FONT_3X5
...
static uint8_t battery_widget_number = CONFIG_ARCADE_BATTERY_WIDGET_NUMBER;
...
static void announce_connection(struct peripheral_battery_state state) {
    ...
    if ((was == 0) != (state.level == 0)) {
        arcade_sound_connected(state.level != 0);
    }
}
