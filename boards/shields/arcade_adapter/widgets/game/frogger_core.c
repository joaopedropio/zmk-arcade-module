...
static bool off_panel(int mid) {
    return mid - FR_FROG_W / 2 < 0 || mid + FR_FROG_W / 2 >= ARC_PANEL;
}
...
static int ride_left(const fr_game *g, int row, int mid) {
    ...
    int slack = drift > 0 ? ARC_PANEL - 1 - FR_FROG_W / 2 - mid : mid - FR_FROG_W / 2;
...
static int move_value(const fr_game *g, int dcol, int drow, int x8, int row, int commit) {
     ...
     */
    if (is_river(row)) {
        int slack = mid < ARC_PANEL / 2 ? mid : ARC_PANEL - 1 - mid;
        if (slack < 3 * FR_EDGE) {
