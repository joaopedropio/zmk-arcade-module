...
void dk_render_default_palette(dk_palette *p) {
    ...
    p->girder = arc_rgb565(0xd83030);
    p->ladder = arc_rgb565(0x30c0f0);
    p->climber = arc_rgb565(0xe03030);
    p->climber_trim = arc_rgb565(0x3060e0);
    p->barrel = arc_rgb565(0xd08020);
    p->ape = arc_rgb565(0xa05820);
    p->ape_trim = arc_rgb565(0xf0c088);
    p->lady = arc_rgb565(0xf060a0);
    p->hammer = arc_rgb565(0xf0d040);
    p->oil = arc_rgb565(0x2060d0);
...
static bool band_hits(int y, int h) { return y < arc_by + arc_bh && y + h > arc_by; }
...
static void draw_ladder(const dk_ladder *l) {
    ...
    for (int y = top; y < bottom; y++) {
        ...
        arc_put(x - DK_LADDER_W / 2, y, pal.ladder);
        arc_put(x + DK_LADDER_W / 2 - 1, y, pal.ladder);
    }
    for (int y = top + 2; y < bottom - 1; y += DK_RUNG) {
        if (l->broken && y >= gap0 && y < gap1) {
            continue;
        }
        arc_fill(x - DK_LADDER_W / 2, y, DK_LADDER_W, 1, dim(pal.ladder));
    }
}
...
static void draw_girder(int f) {
    int x0 = arc_bx, x1 = arc_bx + arc_bw;
    ...
    for (int x = x0; x < x1; x++) {
        ...
        arc_fill(x, y, 1, DK_BEAM, pal.girder);
        arc_put(x, y, lift(pal.girder));
        if ((x & 7) == 0) {
            arc_put(x, y + 2, dim(pal.girder));
        }
    }
}
...
static void draw_drum(const dk_game *g) {
    ...
    if (band_hits(top, DK_DRUM_H)) {
        arc_fill(DK_DRUM_X, top, DK_DRUM_W, DK_DRUM_H, pal.oil);
        arc_fill(DK_DRUM_X, top, DK_DRUM_W, 2, lift(pal.oil));
        for (int i = 1; i < 4; i++) {
            arc_fill(DK_DRUM_X, top + i * DK_DRUM_H / 4, DK_DRUM_W, 1, dim(pal.oil));
        }
    }
    ...
    for (int i = 0; i < h; i++) {
        ...
        arc_fill(DK_DRUM_X + 3 + (i & 1) + (DK_DRUM_W - 6 - w) / 2, y + i, w, 1,
...
static void paint_site(const dk_game *g) {
    ...
    if (band_hits(DK_TOP - 1, 1)) {
        arc_fill(arc_bx, DK_TOP - 1, arc_bw, 1, dim(pal.girder));
    }
...
static void draw_hero(const dk_game *g, int cx, int feet) {
    ...
    for (int j = 0; j < DK_ART_H; j++) {
        for (int i = 0; i < DK_ART_W; i++) {
            ...
            char c = art[j][src];
            if (c == '#') {
                arc_put(x0 + i, y0 + j, pal.climber);
            } else if (c == 'o') {
                arc_put(x0 + i, y0 + j, pal.climber_trim);
            }
        }
    }
    ...
    if (h->hammer > 0) {
        ...
        int hy = up ? y0 - 7 : y0 + 3;
        arc_fill(hx, hy, 10, 5, pal.hammer);
        arc_fill(hx + 3, hy + 5, 4, 4, dim(pal.hammer));
    }
}
...
static void draw_barrel(const dk_barrel *b) {
    ...
    arc_fill(x0 + 1, y0, DK_BARREL_W - 2, DK_BARREL_H, pal.barrel);
    arc_fill(x0, y0 + 2, 1, DK_BARREL_H - 4, pal.barrel);
    arc_fill(x0 + DK_BARREL_W - 1, y0 + 2, 1, DK_BARREL_H - 4, pal.barrel);

    if (b->state == DK_B_LADDER) {
        /* end on, coming down a ladder: two rings rather than two staves */
        arc_fill(x0 + 2, y0 + 2, DK_BARREL_W - 4, DK_BARREL_H - 4, dim(pal.barrel));
        return;
    }
    int phase = ((cx + bottom) / 3) & 3;
    for (int i = 0; i < 2; i++) {
        int x = x0 + 2 + ((phase + i * 5) % (DK_BARREL_W - 3));
        arc_fill(x, y0 + 1, 1, DK_BARREL_H - 2, dim(pal.barrel));
    }
    arc_fill(x0 + 1, y0 + 1, DK_BARREL_W - 2, 1, lift(pal.barrel));
}
...
static void draw_ape(const dk_game *g) {
    ...
    arc_fill(x + 8, y + 9, DK_APE_W - 16, 13, pal.ape);
    arc_fill(x + 11, y + 12, DK_APE_W - 22, 8, pal.ape_trim);
    arc_fill(x + 10, y, 14, 10, pal.ape);
    arc_fill(x + 13, y + 4, 8, 5, pal.ape_trim);
    arc_fill(x + 12, y + 2, 2, 2, pal.site);
    arc_fill(x + 20, y + 2, 2, 2, pal.site);
    arc_fill(x + 7, y + 1, 3, 4, pal.ape);
    arc_fill(x + 24, y + 1, 3, 4, pal.ape);
    arc_fill(x, arms, 8, 11, pal.ape);
    arc_fill(x + DK_APE_W - 8, arms, 8, 11, pal.ape);
    arc_fill(x + 8, y + 22, 7, 6, pal.ape);
    arc_fill(x + DK_APE_W - 15, y + 22, 7, 6, pal.ape);
}
...
static void draw_lady(const dk_game *g) {
    ...
    arc_fill(x + 3, y, 6, 5, dim(pal.lady));
    arc_fill(x + 4, y + 2, 4, 3, lift(pal.lady));
    arc_fill(x + 4, y + 5, 4, 3, pal.lady);
    for (int i = 0; i < 7; i++) {
        arc_fill(x + 4 - i / 2, y + 8 + i, 4 + 2 * (i / 2), 1, pal.lady);
    }
    ...
    for (int i = 0; i < 2; i++) {
        ...
        arc_fill(hx, hy + 1, 4, 3, pal.lady);
        arc_fill(hx + 1, hy, 1, 1, pal.lady);
        arc_fill(hx + 2, hy, 1, 1, pal.lady);
        arc_fill(hx + 1, hy + 4, 2, 1, pal.lady);
    }
}
...
static void draw_hammer(int i) {
    ...
    arc_fill(x - 5, y, 10, 5, pal.hammer);
    arc_fill(x - 1, y + 5, 3, 6, dim(pal.hammer));
}
...
static void draw_spin(const dk_game *g, int cx, int cy) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        for (int t = -r; t <= r; t++) {
            arc_put(cx + dx * t, cy + dy * t, pal.climber);
        }
    }
    arc_fill(cx - 1, cy - 1, 3, 3, pal.climber_trim);
}
...
static void draw_hud(const dk_game *g) {
    ...
    arc_text(DK_HUD_X, DK_HUD_Y, 2, pal.hud, buf);

    for (int i = 0; i < g->lives && i < 3; i++) {
        int x = DK_LIVES_X + i * 8;
        arc_fill(x + 1, DK_LIVES_Y, 4, 3, pal.climber);
        arc_fill(x, DK_LIVES_Y + 3, 6, 4, pal.climber_trim);
        arc_fill(x + 1, DK_LIVES_Y + 7, 2, 2, pal.climber);
        arc_fill(x + 3, DK_LIVES_Y + 7, 2, 2, pal.climber);
    }

    arc_digits(g->level, 2, buf);
    arc_text(DK_LEVEL_X, DK_LEVEL_Y, 1, pal.hud, buf);
}
...
static void draw_clock(const dk_game *g) {
    ...
    arc_fill(DK_CLOCK_X, DK_CLOCK_Y, DK_CLOCK_W, DK_CLOCK_H, dim(pal.site));
    arc_fill(DK_CLOCK_X, DK_CLOCK_Y, w, DK_CLOCK_H,
...
static void draw_banner(const dk_game *g) {
    ...
    if (word == NULL || !band_hits(DK_BANNER_Y - 4, ARC_GLYPH_H * DK_BANNER_SCALE + 8)) {
        return;
    }
    int w = arc_text_w(word, DK_BANNER_SCALE);
    arc_fill((ARC_PANEL - w) / 2 - 4, DK_BANNER_Y - 4, w + 8, ARC_GLYPH_H * DK_BANNER_SCALE + 8,
            pal.site);
    arc_text((ARC_PANEL - w) / 2, DK_BANNER_Y, DK_BANNER_SCALE, pal.hud, word);
}
...
static void repaint_text(const dk_game *g) {
    ...
    if (memcmp(buf, prev_hud, 9) != 0) {
        ...
        paint(g, 0, 0, ARC_PANEL, DK_TOP - 1);
    }
    ...
    if (strcmp(now, prev_banner) != 0) {
        ...
        int was = arc_text_w(prev_banner, DK_BANNER_SCALE);
        int is = arc_text_w(now, DK_BANNER_SCALE);
        int w = (was > is ? was : is) + 10;
        memcpy(prev_banner, now, strlen(now) + 1);
        paint(g, (ARC_PANEL - w) / 2, DK_BANNER_Y - 5, w, ARC_GLYPH_H * DK_BANNER_SCALE + 10);
    }
}
