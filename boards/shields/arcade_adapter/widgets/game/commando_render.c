...
void cm_render_default_palette(cm_palette *p) {
    p->sky = arc_rgb565(0x2c4a72);
    p->hill = arc_rgb565(0x3f5f4a);
    p->ground = arc_rgb565(0x6b5233);
    p->edge = arc_rgb565(0x9e8b52);
    p->hero = arc_rgb565(0xd8e0c0);
    p->hero_trim = arc_rgb565(0x2f6b2f);
    p->grunt = arc_rgb565(0xc44a3a);
    p->grunt_trim = arc_rgb565(0x2a1a14);
    p->shot = arc_rgb565(0xffe066);
    p->grenade = arc_rgb565(0x4a4a4a);
    p->boom = arc_rgb565(0xff7a1f);
    p->crate = arc_rgb565(0x00c8b4);
...
static void draw_column(const cm_game *g, int x) {
    ...
    col_span(x, CM_HORIZON, surf == CM_PIT ? ARC_PANEL : surf, haze);
    if (surf == CM_PIT) {
        return;
    }
    col_span(x, surf, surf + CM_EDGE, pal.edge);
    col_span(x, surf + CM_EDGE, ARC_PANEL, pal.ground);
    ...
    for (int j = -CM_SUN_R; j <= CM_SUN_R; j++) {
        for (int i = -CM_SUN_R; i <= CM_SUN_R; i++) {
            if (i * i + j * j <= CM_SUN_R * CM_SUN_R) {
                arc_put(CM_SUN_X + i, CM_SUN_Y + j, face);
            }
        }
    }
}
...
static void draw_hero(const cm_game *g) {
    ...
    if (g->airborne) {
        arc_fill(cx - 6, fy - 9, 5, 7, pal.hero);
        arc_fill(cx + 1, fy - 11, 5, 7, pal.hero);
    } else {
        arc_fill(cx - 6 + (stride ? 3 : 0), fy - 9, 4, 9, pal.hero);
        arc_fill(cx + 2 - (stride ? 3 : 0), fy - 9, 4, 9, pal.hero);
    }
    arc_fill(cx - 5, fy - 20, 10, 11, pal.hero);
    arc_fill(cx - 5, fy - 20, 10, 2, shade(pal.hero, 5, 4));

    /* head, then the helmet over it with a brim out front */
    arc_fill(cx - 4, fy - 26, 8, 6, pal.hero);
    arc_fill(cx - 5, fy - 26, 10, 3, pal.hero_trim);
    arc_fill(cx + 4, fy - 24, 3, 2, pal.hero_trim);

    arc_fill(cx - 2, chest - 1, 12, 3, pal.hero_trim);
    if (cm_firing(g)) {
        arc_fill(cx + 10, chest - 2, 4, 5, pal.shot);
    }
}
...
static void draw_foe(const cm_game *g, const cm_foe *f) {
    ...
    if (f->kind == CM_F_GUNNER) {
        arc_fill(cx - 7, fy - 14, 14, 14, pal.grunt);
        arc_fill(cx - 7, fy - 14, 14, 3, shade(pal.grunt, 5, 4));
        arc_fill(cx - 4, fy - 20, 8, 6, pal.grunt);
        arc_fill(cx - 4, fy - 20, 8, 2, pal.grunt_trim);
        arc_fill(cx - 12, fy - 9, 7, 3, pal.grunt_trim);
        return;
    }

    arc_fill(cx - 5 + (stride ? -3 : 0), fy - 9, 4, 9, pal.grunt);
    arc_fill(cx + 1 + (stride ? 3 : 0), fy - 9, 4, 9, pal.grunt);
    arc_fill(cx - 5, fy - 20, 10, 11, pal.grunt);
    arc_fill(cx - 4, fy - 26, 8, 6, pal.grunt);
    arc_fill(cx - 6, fy - 26, 10, 3, pal.grunt_trim);
    arc_fill(cx - 11, fy - 15, 10, 3, pal.grunt_trim);
}
...
static void draw_crate(const cm_game *g, const cm_crate *c) {
    ...
    arc_fill(cx - 6, fy - 12, 12, 12, pal.crate);
    arc_fill(cx - 6, fy - 12, 12, 2, shade(pal.crate, 5, 4));
    arc_fill(cx - 6, fy - 3, 12, 3, shade(pal.crate, 1, 2));
    arc_fill(cx - 1, fy - 10, 2, 8, shade(pal.crate, 1, 3));
    arc_fill(cx - 5, fy - 7, 10, 2, shade(pal.crate, 1, 3));
}

static void draw_shot(const cm_game *g, const cm_shot *s) {
    int cx = screen_x(g, s->wx);

    arc_fill(s->vx > 0 ? cx - 4 : cx, s->y - 1, 5, 2, pal.shot);
    arc_fill(cx - 1, s->y - 1, 2, 2, shade(pal.shot, 5, 4));
}

static void draw_nade(const cm_game *g, const cm_nade *n) {
    int cx = screen_x(g, n->wx), cy = CM_PX(n->y);

    arc_fill(cx - 2, cy - 3, 5, 6, pal.grenade);
    arc_fill(cx - 1, cy - 4, 3, 2, shade(pal.grenade, 5, 3));
}
...
static void draw_boom(const cm_game *g, const cm_boom *b) {
    ...
    for (int j = -r; j <= r; j++) {
        for (int i = -r; i <= r; i++) {
            ...
            arc_put(cx + i, cy + j, ((i + j) & 2) ? pal.boom : shade(pal.boom, 5, 4));
        }
    }
}
...
static void draw_mark(int x, int y, int which) {
    if (which == 0) { /* a life, as the trooper's helmet */
        arc_fill(x, y + 2, 9, 3, pal.hero_trim);
        arc_fill(x + 1, y, 7, 2, pal.hero_trim);
        arc_fill(x + 1, y + 5, 7, 3, pal.hero);
        return;
    }
    arc_fill(x + 2, y + 2, 5, 6, pal.grenade); /* and a grenade */
    arc_fill(x + 3, y, 3, 2, shade(pal.grenade, 5, 3));
}
...
static void draw_hud(const cm_game *g) {
    ...
    arc_text(CM_HUD_X, CM_HUD_Y, 2, pal.hud, buf);

    for (int i = 0; i < g->lives && i < 4; i++) {
        draw_mark(ARC_PANEL - CM_ICON - 4 - i * (CM_ICON + 3), CM_HUD_Y + 2, 0);
    }
    int gx = CM_GRENADE_X;
    draw_mark(gx, CM_HUD_Y + 2, 1);
    arc_digits(g->nades, 1, buf);
    arc_text(gx + CM_ICON + 2, CM_HUD_Y + 2, 2, pal.hud, buf);
}
...
static void draw_banner(const cm_game *g) {
    ...
    if (word != NULL) {
        arc_text((ARC_PANEL - arc_text_w(word, CM_BANNER_SCALE)) / 2, CM_BANNER_Y,
...
static void paint_band(const cm_game *g, int x0, int y0, int w, int h) {
    ...
    arc_fill(x0, 0, w, CM_OY, shade(pal.ground, 1, 3));
...
static uint8_t prev_surf[ARC_PANEL];
static uint8_t prev_hill[ARC_PANEL];
...
static void repaint_text(const cm_game *g) {
    ...
    if (strcmp(now, prev_banner) != 0) {
        ...
        int was = arc_text_w(prev_banner, CM_BANNER_SCALE);
        int is = arc_text_w(now, CM_BANNER_SCALE);
        int w = (was > is ? was : is) + 2;
        memcpy(prev_banner, now, strlen(now) + 1);
        paint(g, (ARC_PANEL - w) / 2, CM_BANNER_Y - 1, w, ARC_GLYPH_H * CM_BANNER_SCALE + 2);
    }
}
