...
/*
 * Everything here stamps into whatever rectangle paint_band() is filling and
 * lets panel.h clip it, rather than asking "what colour is this pixel" the way
 * the maze's renderer does.  A meteor is a few hundred pixels in a band that
 * may be the whole panel, so walking the sprites is much cheaper than walking
 * the pixels - and the answer is the same either way, because the stamps go
 * down in a fixed order and none of them reads back what is already there.
 *
...
void ss_render_default_palette(ss_palette *p) {
    p->space = arc_rgb565(0x05060f);
    p->star = arc_rgb565(0x8899bb);
    p->ship = arc_rgb565(0x6ee7ff);
    p->trim = arc_rgb565(0xffffff);
    p->thruster = arc_rgb565(0xff8a1f);
    p->bullet = arc_rgb565(0xfff36b);
    p->rock = arc_rgb565(0x5a5f7a);
    p->rock_edge = arc_rgb565(0xa3adc9);
    p->blast = arc_rgb565(0xff5a2b);
    p->power = arc_rgb565(0x39ff9e);
 ...
 */
static int cross(int ax, int ay, int bx1, int by1, int px, int py) {
    return (bx1 - ax) * (py - ay) - (by1 - ay) * (px - ax);
}
...
static void draw_rock(const ss_rock *rk) {
    ...
    for (int y = cy - r; y <= cy + r; y++) {
        if (y < arc_by || y >= arc_by + arc_bh) {
            continue;
        }
        for (int x = cx - r; x <= cx + r; x++) {
            if (x < arc_bx || x >= arc_bx + arc_bw) {
                continue;
            }
            int px = rock_px(rk, x - cx, y - cy);
            if (px != SS_RPX_OUT) {
                arc_put(x, y, px == SS_RPX_EDGE ? pal.rock_edge : pal.rock);
            }
        }
    }
}
...
static void draw_shot(const ss_shot *s) {
    ...
    arc_fill(hx, hy, 2, 2, pal.bullet);
    for (int k = 1; k <= SS_TAIL; k++) {
        arc_put(hx - (int)s->vx * k / (4 * SS_SUB), hy - (int)s->vy * k / (4 * SS_SUB),
            pal.bullet);
    }
}
...
static void draw_drop(const ss_game *g) {
    ...
    for (int y = cy - r; y <= cy + r; y++) {
        for (int x = cx - r; x <= cx + r; x++) {
            ...
            if (d <= r && d > r - 2) {
                arc_put(x, y, pal.power);
            }
        }
    }
    char word[2] = {POWER_LETTER[g->drop.kind], '\0'};
    arc_text(cx - 2, cy - 3, 1, pal.power, word);
}
    ...
    for (int y = y0; y <= y1; y++) {
        for (int x = x0; x <= x1; x++) {
            if (g->ship.thrusting && in_tri(x, y, flame)) {
                arc_put(x, y, pal.thruster);
            }
            if (!in_tri(x, y, hull) || in_tri(x, y, notch)) {
                continue;
            }
            arc_put(x, y, in_tri(x, y, pit) ? pal.trim : pal.ship);
        }
    }
...
static void draw_life(int x, int y) {
    for (int j = 0; j < 5; j++) {
        ...
        arc_fill(x + (7 - w) / 2, y + j, w, 1, pal.ship);
    }
}
...
static void draw_hud(const ss_game *g) {
    ...
    arc_text(SS_HUD_X, SS_HUD_Y, 2, pal.hud, buf);

    for (int i = 0; i < g->lives && i < 5; i++) {
        draw_life(ARC_PANEL - 9 - i * 10, SS_HUD_Y + 4);
    }

    const char *power = ss_power_name(g);
    if (power != NULL) {
        arc_text(ARC_PANEL - SS_HUD_X - arc_text_w(power, 1), SS_FOOT_Y, 1, pal.power, power);
    }
}
...
static void draw_banner(const ss_game *g) {
    ...
    arc_text((ARC_PANEL - arc_text_w(word, SS_BANNER_SCALE)) / 2, SS_BANNER_Y, SS_BANNER_SCALE,
              pal.hud, word);
}
...
static void paint_band(const ss_game *g, int x0, int y0, int w, int h) {
    ...
    arc_fill(x0, y0, w, h, pal.space);

    for (int i = 0; i < SS_STARS; i++) {
        if (ss_star_lit(g, i)) {
            arc_put(g->stars[i].x, g->stars[i].y, pal.star);
        }
    }
...
static void repaint_text(const ss_game *g) {
    ...
    if (strcmp(buf, prev_hud) != 0) {
        ...
        paint(g, 0, 0, ARC_PANEL, SS_HUD_Y + 2 * ARC_GLYPH_H + 2);
    }

    foot_key(g, buf);
    if (strcmp(buf, prev_foot) != 0) {
        memcpy(prev_foot, buf, strlen(buf) + 1);
        paint(g, 0, SS_FOOT_Y - 1, ARC_PANEL, ARC_GLYPH_H + 2);
    }
    ...
    if (strcmp(now, prev_banner) != 0) {
        ...
        int was = arc_text_w(prev_banner, SS_BANNER_SCALE);
        int is = arc_text_w(now, SS_BANNER_SCALE);
        int w = (was > is ? was : is) + 2;
        memcpy(prev_banner, now, strlen(now) + 1);
        paint(g, (ARC_PANEL - w) / 2, SS_BANNER_Y - 1, w, ARC_GLYPH_H * SS_BANNER_SCALE + 2);
    }
}
