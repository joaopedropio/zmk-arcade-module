...
void fr_render_default_palette(fr_palette *p) {
    p->water = arc_rgb565(0x1030a8);
    p->road = arc_rgb565(0x101014);
    p->bank = arc_rgb565(0x1f7a3a);
    p->hedge = arc_rgb565(0x0f5a2a);
    p->frog = arc_rgb565(0x7bf03c);
    p->frog_eye = arc_rgb565(0xffffff);
    p->log = arc_rgb565(0x8a5a2b);
    p->turtle = arc_rgb565(0xf0a020);
    p->car = arc_rgb565(0xe03c3c);
    p->truck = arc_rgb565(0xd8d8e0);
    p->splat = arc_rgb565(0xffffff);
    p->fly = arc_rgb565(0xff40c0);
...
static void dashes(int y, int stride, int len, int phase, uint16_t c) {
    ...
    int first = arc_bx - len + 1 - phase;
    first = (first > 0 ? first / stride : (first - stride + 1) / stride) * stride + phase;
    for (int x = first; x < arc_bx + arc_bw; x += stride) {
        arc_fill(x, y, len, 1, c);
    }
}
...
static void paint_ground(void) {
    if (band_hits(0, FR_TOP)) {
        arc_fill(arc_bx, 0, arc_bw, FR_TOP, pal.road);
    }
    if (band_hits(FR_FOOT, ARC_PANEL - FR_FOOT)) {
        arc_fill(arc_bx, FR_FOOT, arc_bw, ARC_PANEL - FR_FOOT, pal.road);
    }
    ...
    for (int row = 0; row < FR_ROWS; row++) {
        ...
        arc_fill(arc_bx, y, arc_bw, FR_CELL, base);
    ...
    for (int b = 0; b < FR_BAYS; b++) {
        ...
        arc_fill(x, y, FR_CELL, FR_CELL, pal.water);
        arc_fill(x, y, FR_CELL, 1, dim(pal.hedge));
    }
}
...
static void draw_frog(int cx, int cy, int facing, bool hopping, uint16_t body,
    ...
    for (int v = 0; v < FR_ART; v++) {
        for (int u = 0; u < FR_ART; u++) {
            ...
            arc_put(x0 + x, y0 + y, c == 'o' ? eye : body);
        }
    }
}
...
static void draw_log(int x, int y, int w, int phase) {
    for (int j = 0; j < FR_SPRITE_H; j++) {
        ...
        arc_fill(x + inset, y + j, w - 2 * inset, 1, pal.log);
    }
    /* grain along it, and the two cut ends */
    for (int j = 3; j < FR_SPRITE_H - 2; j += 3) {
        for (int i = 4 + (phase + j) % 5; i < w - 4; i += 7) {
            arc_fill(x + i, y + j, 3, 1, dim(pal.log));
        }
    }
    arc_fill(x + 2, y + 2, 1, FR_SPRITE_H - 4, dim(pal.log));
    arc_fill(x + w - 3, y + 2, 1, FR_SPRITE_H - 4, dim(pal.log));
}
...
static void draw_raft(int x, int y, int w, int sunk, bool rightward) {
    ...
    for (int t = 0; t < w / FR_W_TURT; t++) {
        ...
        arc_fill(rightward ? cx + a : cx - a - 1, cy - 1, 2, 2, shell);
        for (int j = -b; j <= b; j++) {
            for (int i = -a; i <= a; i++) {
                ...
                arc_put(cx + i, cy + j, q * 4 <= span ? lift(shell) : shell);
            }
        }
    }
}
...
static void draw_car(int x, int y, int w, int kind, bool rightward) {
    ...
    if (kind == FR_K_TRUCK) {
        ...
        arc_fill(box, y + 1, w - cab - 2, FR_SPRITE_H - 2, body);
        arc_fill(nose, y, cab, FR_SPRITE_H, body);
        arc_fill(nose + 2, y + 2, cab - 4, 3, glass);
        for (int i = 4; i < w - cab - 6; i += 6) {
            arc_fill(box + i, y + 3, 3, FR_SPRITE_H - 6, glass);
        }
        return;
    }
    ...
    for (int j = 0; j < FR_SPRITE_H; j++) {
        ...
        arc_fill(x + inset + (rightward ? 0 : lead), y + j, w - 2 * inset - lead, 1, body);
    }
    arc_fill(x + w / 2 - 3, y + 2, 6, FR_SPRITE_H - 4, glass);
}

/* the fly, which is worth going out of the way for and has to look like it */
static void draw_fly(int cx, int cy) {
    arc_fill(cx - 2, cy - 1, 5, 3, pal.fly);
    arc_fill(cx - 4, cy - 2, 2, 1, lift(pal.fly));
    arc_fill(cx + 3, cy - 2, 2, 1, lift(pal.fly));
    arc_put(cx, cy - 2, pal.fly);
}
...
static void draw_splat(int cx, int cy, int age) {
    ...
    for (int i = -r; i <= r; i++) {
        arc_put(cx + i, cy, pal.splat);
        arc_put(cx, cy + i, pal.splat);
        if ((i & 1) == 0) {
            arc_put(cx + i, cy + i, pal.splat);
            arc_put(cx + i, cy - i, pal.splat);
        }
    }
}
...
static void draw_life(int x, int y) {
    arc_fill(x, y + 2, 8, 5, pal.frog);
    arc_fill(x + 1, y, 2, 2, pal.frog);
    arc_fill(x + 5, y, 2, 2, pal.frog);
    arc_put(x + 1, y, pal.frog_eye);
    arc_put(x + 6, y, pal.frog_eye);
}
...
static void draw_hud(const fr_game *g) {
    ...
    arc_text(FR_HUD_X, FR_HUD_Y, 2, pal.hud, buf);

    for (int i = 0; i < g->lives && i < 5; i++) {
        draw_life(ARC_PANEL - 10 - i * 10, FR_HUD_Y + 3);
    }
}
...
static void draw_clock(const fr_game *g) {
    ...
    if (w > 0) {
        ...
        arc_fill(FR_CLOCK_X, FR_CLOCK_Y, w, FR_CLOCK_H,
                w * 4 < FR_CLOCK_W ? pal.splat : pal.bank);
    }

    char buf[6] = {'L', 'V', ' ', 0, 0, 0};
    arc_digits(g->level, 2, buf + 3);
    arc_text(ARC_PANEL - FR_HUD_X - arc_text_w(buf, 1), FR_CLOCK_Y - 1, 1, pal.hud, buf);
}
...
static void draw_banner(const fr_game *g) {
    ...
    int w = arc_text_w(word, FR_BANNER_SCALE);
    arc_fill((ARC_PANEL - w) / 2 - 4, FR_BANNER_Y - 4, w + 8, ARC_GLYPH_H * FR_BANNER_SCALE + 8,
            pal.road);
    arc_text((ARC_PANEL - w) / 2, FR_BANNER_Y, FR_BANNER_SCALE, pal.hud, word);
}
...
static void paint_band(const fr_game *g, int x0, int y0, int w, int h) {
    ...
    for (int row = 0; row < FR_ROWS; row++) {
        ...
        for (int i = 0; i < l->count; i++) {
            ...
            if (x >= arc_bx + arc_bw || x + (int)l->span[i] <= arc_bx) {
                continue;
            }
            switch (l->kind) {
            case FR_K_LOG:
...
static void level_key(const fr_game *g, char *buf) { arc_digits(g->level, 2, buf); }
...
static void repaint_text(const fr_game *g) {
    ...
    if (strcmp(buf, prev_hud) != 0) {
        ...
        paint(g, 0, 0, ARC_PANEL, FR_TOP);
    }

    level_key(g, buf);
    if (strcmp(buf, prev_level) != 0) {
        memcpy(prev_level, buf, strlen(buf) + 1);
        paint(g, ARC_PANEL / 2, FR_FOOT, ARC_PANEL / 2, ARC_PANEL - FR_FOOT);
    }
    ...
    if (strcmp(now, prev_banner) != 0) {
        ...
        int was = arc_text_w(prev_banner, FR_BANNER_SCALE);
        int is = arc_text_w(now, FR_BANNER_SCALE);
        int w = (was > is ? was : is) + 10;
        memcpy(prev_banner, now, strlen(now) + 1);
        paint(g, (ARC_PANEL - w) / 2, FR_BANNER_Y - 5, w, ARC_GLYPH_H * FR_BANNER_SCALE + 10);
    }
}
