...
void fg_render_default_palette(fg_palette *p) {
    p->sky = arc_rgb565(0x2a1a4a);
    p->crowd = arc_rgb565(0x53306b);
    p->floor = arc_rgb565(0x8a5a2b);
    p->body[0] = arc_rgb565(0xf2f2f2);
    p->trim[0] = arc_rgb565(0xd42a2a);
    p->body[1] = arc_rgb565(0xffcf3f);
    p->trim[1] = arc_rgb565(0x1e73ff);
    p->spark = arc_rgb565(0xfff4c2);
    p->fireball = arc_rgb565(0x4fd8ff);
    p->health = arc_rgb565(0xffe000);
    p->health_lost = arc_rgb565(0xd03020);
...
static void draw_stage(const fg_game *g, int x0, int w) {
    arc_fill(x0, FG_OY, w, FG_HORIZON - FG_OY, pal.sky);
    arc_fill(x0, FG_HORIZON, w, FG_FLOOR - FG_HORIZON, pal.crowd);

    uint16_t lit = shade(pal.crowd, 5, 4), dark = shade(pal.crowd, 2, 3);
    for (int y = FG_HORIZON + 8; y < FG_FLOOR - 24; y += 26) {
        for (int x = 6; x < ARC_PANEL; x += 26) {
            arc_fill(x, y, 12, 16, dark);
            arc_fill(x, y, 12, 2, lit);
        }
    }
    arc_fill(x0, FG_HORIZON - 2, w, 2, lit);
    ...
    arc_fill(x0, FG_FLOOR, w, ARC_PANEL - FG_FLOOR, face);
    arc_fill(x0, FG_FLOOR, w, 3, shade(face, 5, 4));
    for (int x = 10; x < ARC_PANEL; x += 34) {
        arc_fill(x, FG_FLOOR + 4, 2, ARC_PANEL - FG_FLOOR - 4, shade(face, 3, 4));
    }

    /* and the band the readout is written on */
    arc_fill(x0, 0, w, FG_OY, shade(pal.crowd, 1, 3));
}
...
static void draw_down(int cx, int fy, int s, uint16_t body, uint16_t trim) {
    ...
    arc_fill(cx - 16, fy - 8, 32, 8, body);
    arc_fill(cx - 16, fy - 8, 32, 2, shade(body, 5, 4));
    arc_fill(hx, fy - 10, 10, 10, body);
    arc_fill(hx, fy - 7, 10, 3, trim);
}
...
static void draw_kick(int cx, int hip, int mid, int s, int reach, uint16_t body,
    ...
    for (int i = 0; i < reach; i++) {
        ...
        arc_fill(cx + (s > 0 ? i : -1 - i), y - th / 2, 1, th, body);
    }
    ...
    arc_fill(bx, by, bw, FG_BOOT_H, trim);
    arc_fill(bx, by + FG_BOOT_H - 3, bw, 3, shade(trim, 1, 2));
}
...
static void draw_fighter(const fg_game *g, int who) {
    ...
    if (kicking) {
        ...
        arc_fill(lx, fy - leg, 7, leg, body);
        arc_fill(lx, fy - 3, 7, 3, shade(body, 1, 2));
    } else {
        arc_fill(cx - 7 + step, fy - leg, 6, leg, body);
        arc_fill(cx + 1 - step, fy - leg, 6, leg, body);
        arc_fill(cx - 7 + step, fy - 3, 6, 3, shade(body, 1, 2));
        arc_fill(cx + 1 - step, fy - 3, 6, 3, shade(body, 1, 2));
    }

    arc_fill(cx - 7, ty, 14, torso, body);
    arc_fill(cx - 7, ty, 14, 2, shade(body, 5, 4));
    arc_fill(cx - 7, fy - leg - 3, 14, 3, trim);

    /* the head, carried forward of the middle, and further back when hurt */
    int lean = f->state == FG_S_HURT ? -3 : 2;
    int hx = cx - 6 + lean * s;
    arc_fill(hx, hy, 12, head, body);
    arc_fill(hx, hy + 3, 12, 3, trim);
    arc_fill(hx + (s > 0 ? 8 : 2), hy + 7, 2, 2, shade(body, 1, 3));
    arc_fill(s > 0 ? hx - 4 : hx + 12, hy + 3, 4, 2, trim);

    switch (f->state) {
    case FG_S_BLOCK:
        /* the forearm up in front, which is the whole of what a guard is */
        arc_fill(s > 0 ? cx + 5 : cx - 10, ty + 1, 5, torso - 2, trim);
        break;
    case FG_S_FIRE:
        /* both hands out, and the ball gathering in them on the way */
        arc_fill(s > 0 ? cx + 2 : cx - 11, fy - FG_HIGH_Y - FG_HIGH_H + 2, 9, 8, trim);
        break;
    case FG_S_WIN:
        arc_fill(cx - 10, hy + 2, 4, 10, body);
        arc_fill(cx + 6, hy + 2, 4, 10, body);
        break;
    default:
         ...
         * leg */
        arc_fill(s > 0 ? cx - 10 : cx + 7, ty + 3, 3, 8, body);
        if (reach == 0 || kicking) {
            arc_fill(s > 0 ? cx + 7 : cx - 10, kicking ? ty : ty + 3, 3, 8, body);
        }
        break;
    }
    ...
    if (reach > 0) {
        ...
        if (high) {
            arc_fill(s > 0 ? cx : cx - reach, mid - 2, reach, 4, body);
            arc_fill(s > 0 ? cx + reach - 6 : cx - reach, mid - 4, 6, 8, trim);
        } else {
...
static void draw_spark(const fg_spark *s) {
    ...
    for (int k = -r; k <= r; k++) {
        int t = k < 0 ? -k : k;
        arc_put(s->x + k, s->y, pal.spark);
        arc_put(s->x, s->y + k, pal.spark);
        if (t <= r - 2) {
            arc_put(s->x + k, s->y + k, pal.spark);
            arc_put(s->x + k, s->y - k, pal.spark);
        }
    }
}
...
static void draw_bar(int who, uint8_t health, uint8_t lag) {
    int x = who == 0 ? FG_BAR_X : ARC_PANEL - FG_BAR_X - FG_BAR_W;

    arc_fill(x - 1, FG_BAR_Y - 1, FG_BAR_W + 2, FG_BAR_H + 2, shade(pal.hud, 1, 3));
    arc_fill(x, FG_BAR_Y, FG_BAR_W, FG_BAR_H, shade(pal.health_lost, 1, 3));

    if (lag > 0) {
        int lx = who == 0 ? x : x + FG_BAR_W - lag;
        arc_fill(lx, FG_BAR_Y, lag, FG_BAR_H, pal.health_lost);
    }
    if (health > 0) {
        int hx = who == 0 ? x : x + FG_BAR_W - health;
        arc_fill(hx, FG_BAR_Y, health, FG_BAR_H, pal.health);
        arc_fill(hx, FG_BAR_Y, health, 2, shade(pal.health, 5, 4));
    }
}
    ...
    for (int i = 0; i < 2; i++) {
        ...
        for (int k = 0; k < g->wins[i] && k < FG_ROUNDS_WIN; k++) {
            int x = i == 0 ? FG_BAR_X + k * (FG_PIP + 3)
                           : ARC_PANEL - FG_BAR_X - FG_PIP - k * (FG_PIP + 3);
            arc_fill(x, FG_BAR_Y + FG_BAR_H + 4, FG_PIP, FG_PIP, pal.health);
        }
    }

    /* the clock in whole seconds, which is what a fighting game counts in */
    arc_digits(g->clock / 15, 2, buf);
    arc_text((ARC_PANEL - arc_text_w(buf, 2)) / 2, FG_BAR_Y - 2, 2, pal.hud, buf);
}
...
static void draw_banner(const fg_game *g) {
    ...
    if (word != NULL) {
        arc_text((ARC_PANEL - arc_text_w(word, FG_BANNER_SCALE)) / 2, FG_BANNER_Y,
...
static void text_key(const fg_game *g, int which, char *buf) {
    if (which < FG_T_PIP) {
        ...
        arc_digits(g->f[i].health, 2, buf);
        arc_digits(g->bar[i], 2, buf + 2);
        return;
    }
    if (which < FG_T_CLOCK) {
...
static void text_rect(int which, int *x, int *y, int *w, int *h) {
    if (which < FG_T_CLOCK) {
        int i = which & 1;
        int bx = i == 0 ? FG_BAR_X : ARC_PANEL - FG_BAR_X - FG_BAR_W;
        if (which < FG_T_PIP) {
            *x = bx - 2;
            *y = FG_BAR_Y - 2;
            *w = FG_BAR_W + 4;
        ...
        *x = i == 0 ? FG_BAR_X : ARC_PANEL - FG_BAR_X - *w;
...
static void repaint_text(const fg_game *g) {
    ...
    if (strcmp(now, prev_banner) != 0) {
        ...
        int was = arc_text_w(prev_banner, FG_BANNER_SCALE);
        int is = arc_text_w(now, FG_BANNER_SCALE);
        int w = (was > is ? was : is) + 2;
        memcpy(prev_banner, now, strlen(now) + 1);
        paint(g, (ARC_PANEL - w) / 2, FG_BANNER_Y - 1, w, ARC_GLYPH_H * FG_BANNER_SCALE + 2);
    }
}
...
void fg_render_frame(fg_game *g) {
    ...
    if (g->flash != prev_flash) {
        prev_flash = g->flash;
        paint(g, 0, FG_FLOOR, ARC_PANEL, ARC_PANEL - FG_FLOOR);
    }
