...
int dk_floor_y(int f, int x) {
    ...
    return DK_BASE - f * DK_RISE + DK_DIR(f) * ((x * DK_DROP) / ARC_PANEL - DK_DROP / 2);
}
...
static int clamp_x(int x8) {
    ...
    if (x8 > (ARC_PANEL - 1 - DK_EDGE) * DK_SUB) {
        return (ARC_PANEL - 1 - DK_EDGE) * DK_SUB;
    }
    return x8;
}
...
static void step_barrel(dk_game *g, dk_barrel *b) {
    ...
    if (now >= DK_EDGE && now <= ARC_PANEL - 1 - DK_EDGE) {
    ...
    int edge = now < DK_EDGE ? DK_EDGE : ARC_PANEL - 1 - DK_EDGE;
...
static bool barrel_at(const dk_game *g, const dk_barrel *b, int t, int *floor, int *x,
                      bool *span) {
    int v = geared(g, DK_ROLL);
    int lo = DK_EDGE * DK_SUB, hi = (ARC_PANEL - 1 - DK_EDGE) * DK_SUB;
    int fl, x8, dir;
