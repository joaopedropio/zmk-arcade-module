/*
 * Arcade dongle - renderer (portable).
 *
...
_Static_assert(PM_BLIT_MAX <= ARC_BAND_PX, "a maze blit outgrew panel.h's band");
...
void pm_render_default_palette(pm_palette *p) {
    p->bg = arc_rgb565(0x000000);
    p->wall_fill = arc_rgb565(0x00003c);
    p->wall_edge = arc_rgb565(0x2121de);
    p->wall_flash = arc_rgb565(0xf8f8f8);
    p->house_fill = arc_rgb565(0x00003c);
    p->house_edge = arc_rgb565(0x6d6dff);
    p->door = arc_rgb565(0xffb8ff);
    p->pellet = arc_rgb565(0xffb897);
    p->pac = arc_rgb565(0xffee00);
    p->ghost[0] = arc_rgb565(0xff0000);
    p->ghost[1] = arc_rgb565(0xffb8ff);
    p->ghost[2] = arc_rgb565(0x00ffff);
    p->ghost[3] = arc_rgb565(0xffb852);
    p->fright_body = arc_rgb565(0x2121de);
    p->fright_face = arc_rgb565(0xffffff);
    p->flash_body = arc_rgb565(0xf8f8f8);
    p->flash_face = arc_rgb565(0xff0000);
    p->eye = arc_rgb565(0xffffff);
    p->pupil = arc_rgb565(0x2121de);
}
 ...
 * fill colour, or a wall drawn with ARCADE_WALL_FILL_COLOR set would keep its
 * square corners.
 *
...
static void paint(pm_game *g, int x0, int y0, int w, int h) {
    ...
    arc_blit((uint16_t)(x0 + PM_MARGIN), (uint16_t)(y0 + PM_MARGIN), (uint16_t)w,
            (uint16_t)h, arc_band);
}
...
static void paint_border(const pm_game *g) {
    ...
    paint_band(g, 0, 0, ARC_PANEL, lo);
    paint_band(g, 0, hi, ARC_PANEL, PM_BORDER_FAR);
