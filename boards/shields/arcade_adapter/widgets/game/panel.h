/*
 * Arcade dongle - what the games agree on about the panel.
 *
...
#define ARC_PANEL 240

/*
 * One staging band, shared.  A renderer paints a rectangle into this and hands
 * it to arc_blit(); only one game is ever running, so a buffer each would be
 ...
 */
#define ARC_BAND_PX 5280
extern uint8_t arc_band[ARC_BAND_PX * 2];

/* rgb888 down to the panel's 5-6-5 */
uint16_t arc_rgb565(uint32_t rgb888);

/* implemented by the platform: push w*h RGB565 (big endian) pixels at x,y */
extern void arc_blit(uint16_t x, uint16_t y, uint16_t w, uint16_t h, const uint8_t *pixels);
 ...
 * prologues.
 */
extern int arc_bx, arc_by, arc_bw, arc_bh;

static inline void arc_band_begin(int x, int y, int w, int h) {
    arc_bx = x;
    arc_by = y;
    arc_bw = w;
    arc_bh = h;
...
static inline void arc_put(int x, int y, uint16_t c) {
    if (x < arc_bx || x >= arc_bx + arc_bw || y < arc_by || y >= arc_by + arc_bh) {
        return;
    }
    uint8_t *p = arc_band + 2 * ((y - arc_by) * arc_bw + (x - arc_bx));
    p[0] = (uint8_t)(c >> 8);
    p[1] = (uint8_t)(c & 0xFF);
}

void arc_fill(int x, int y, int w, int h, uint16_t c);
 ...
 */
#define ARC_GLYPH_W 5
#define ARC_GLYPH_H 7
#define ARC_GLYPH_ADV 6 /* one column of air between letters */

/* how wide a string comes out, so it can be centred or right aligned */
int arc_text_w(const char *s, int scale);
/* uppercase letters, digits and space; anything else is drawn as a space */
void arc_text(int x, int y, int scale, uint16_t c, const char *s);
/* a number zero padded to n digits, so a readout never changes width */
void arc_digits(uint32_t value, int digits, char *buf);
