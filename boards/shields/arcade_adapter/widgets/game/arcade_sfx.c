 ...
 * heard over.
 */
static const arc_instrument INSTRUMENTS[] = {
    /* a struck bell: the 2.76 partial is what stops it sounding like an organ */
    [ARC_INST_BELL] = {3, 900, 0, 260, 276, 100, ENV_ONE * 35 / 100, ENV_ONE, false},
    /* a marimba: an octave above, gone almost at once */
    [ARC_INST_PLUCK] = {2, 260, 0, 90, 2, 1, ENV_ONE * 25 / 100, ENV_ONE, true},
    /* a pad: six cents apart, so the two drift in and out of phase */
    [ARC_INST_PAD] = {140, 260, ENV_ONE * 72 / 100, 420, 1006, 1000, ENV_ONE * 85 / 100,
                     ENV_ONE * 45 / 100, true},
    /* and a soft tick */
    [ARC_INST_NOISE] = {1, 130, 0, 50, 1, 1, 0, ENV_ONE, false},
    /*
     ...
     */
    [ARC_INST_CHIME] = {110, 950, 0, 460, 2, 1, ENV_ONE * 12 / 100, ENV_ONE, false},
};

static const int16_t SINE[256] = ARC_SINE_TABLE;

typedef struct {
    bool on;
    const arc_instrument *inst;
    uint32_t phase, step;
...
static arc_tune_id current = ARC_TUNE_NONE;
static uint32_t elapsed;  /* samples since the tune started */
static uint8_t next_note; /* the first note not yet started */

static const arc_tune *tune_of(arc_tune_id id) {
    return (id < ARC_TUNE_COUNT) ? &ARC_TUNES[id] : NULL;
}
...
static bool envelope(arc_voice *v, int32_t *out) {
    const arc_instrument *in = v->inst;
...
static int32_t voice_sample(arc_voice *v) {
    int32_t env;
    ...
    int32_t a;
    if (v->inst == &INSTRUMENTS[ARC_INST_NOISE]) {
...
static void start_note(const arc_note *n) {
    arc_voice *pick = &voices[0];
    ...
    const arc_instrument *in = &INSTRUMENTS[n->inst];
    uint32_t hz = n->hz;
    uint32_t step;
...
void arc_sfx_init(uint32_t sample_rate, uint8_t vol, uint16_t low_hz) {
    rate = sample_rate ? sample_rate : 16000;
    arc_sfx_set_volume(vol);
    arc_sfx_set_bass_floor(low_hz);
    arc_sfx_stop();
}
 ...
 */
void arc_sfx_set_volume(uint8_t vol) {
    if (vol > 100) {
        vol = 100;
    }
...
void arc_sfx_set_bass_floor(uint16_t low_hz) { floor_hz = low_hz; }

void arc_sfx_play(arc_tune_id id) {
    const arc_tune *tune = tune_of(id);
    const arc_tune *playing = tune_of(current);
...
void arc_sfx_stop(void) {
    current = ARC_TUNE_NONE;
    for (int i = 0; i < VOICES; i++) {
        voices[i].on = false;
    }
}

arc_tune_id arc_sfx_playing(void) { return current; }

bool arc_sfx_sounding(void) {
    if (current != ARC_TUNE_NONE) {
        return true;
    }
    for (int i = 0; i < VOICES; i++) {
        if (voices[i].on) {
...
size_t arc_sfx_render(int16_t *out, size_t count) {
    const arc_tune *tune = tune_of(current);
    ...
    return (current == ARC_TUNE_NONE) ? 0 : count;
}
