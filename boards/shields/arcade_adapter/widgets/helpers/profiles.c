...
#define CURRENT_KEY "arcprof/current"
...
typedef struct {
    ...
    uint8_t reserved;
    uint16_t entries;
    char name[ARCADE_PROFILE_NAME_LEN];
    profile_entry entry[ARCADE_SETTING_COUNT];
} profile_record;
...
static uint32_t hashes[ARCADE_SETTING_COUNT];
...
static bool hashes_usable(void) {
    ...
    for (arcade_setting_id id = 0; id < ARCADE_SETTING_COUNT; id++) {
        hashes[id] = name_hash(arcade_settings_describe(id)->name);
    }
    for (arcade_setting_id a = 0; a < ARCADE_SETTING_COUNT; a++) {
        for (arcade_setting_id b = a + 1; b < ARCADE_SETTING_COUNT; b++) {
            if (hashes[a] != hashes[b]) {
                continue;
            }
            LOG_ERR("Profiles are off: \"%s\" and \"%s\" hash alike; rename one",
                    arcade_settings_describe(a)->name, arcade_settings_describe(b)->name);
            hashes_collide = true;
        }
    }
    return !hashes_collide;
...
bool arcade_profile_slot_valid(int slot) { return slot >= 0 && slot < ARCADE_PROFILE_SLOTS; }

static void slot_key(int slot, char *buf, size_t len) {
    snprintf(buf, len, "arcprof/%d", slot);
}
...
static int read_cb(const char *key, size_t len, settings_read_cb read_fn, void *cb_arg,
    ...
    scratch.name[ARCADE_PROFILE_NAME_LEN - 1] = '\0';
    *found = true;
    return 0;
}
...
static int fetch(int slot) {
    ...
    if (!arcade_profile_slot_valid(slot)) {
        return -EINVAL;
    }
    if (!hashes_usable()) {
        return -EEXIST;
...
static void current_load(void) {
    ...
    current_ready = true;
    current_slot = ARCADE_PROFILE_DEFAULT_SLOT;
    settings_load_subtree_direct(CURRENT_KEY, current_read_cb, &found);
    if (found && !arcade_profile_slot_valid(current_slot)) {
...
int arcade_profile_current(void) {
    current_load();
    return current_slot;
}
 ...
 */
int arcade_profile_next(void) {
    int here = arcade_profile_current();

    for (int step = 1; step < ARCADE_PROFILE_SLOTS; step++) {
        int slot = (here + step) % ARCADE_PROFILE_SLOTS;
        if (fetch(slot) == 0) {
            return slot;
        }
    }
...
static int snapshot(int slot, const char *name) {
     ...
     */
    arcade_profile_stage_clear();
    for (arcade_setting_id id = 0; id < ARCADE_SETTING_COUNT; id++) {
        staged.entry[staged.entries].key = hashes[id];
        staged.entry[staged.entries].value = arcade_settings_get(id);
        staged.entries++;
    }
 ...
 */
void arcade_profile_init(void) {
    current_load();

    if (fetch(ARCADE_PROFILE_DEFAULT_SLOT) == -ENOENT) {
        int rc = snapshot(ARCADE_PROFILE_DEFAULT_SLOT, ARCADE_PROFILE_DEFAULT_NAME);
        if (rc) {
     ...
     */
    if (current_slot != ARCADE_PROFILE_DEFAULT_SLOT && fetch(current_slot) != 0) {
        LOG_WRN("Profile %u is gone; the dongle is on the default one", current_slot);
        set_current(ARCADE_PROFILE_DEFAULT_SLOT);
    }
}

int arcade_profile_name(int slot, char *buf, size_t len) {
    int rc = fetch(slot);
     ...
     */
    return slot == arcade_profile_current() ? ARCADE_SETTING_COUNT : (int)scratch.entries;
}

int arcade_profile_read(int slot, arcade_profile_cb fn, void *ctx) {
    /*
     ...
     */
    if (slot == arcade_profile_current()) {
        if (!hashes_usable()) {
            return -EEXIST;
        }
        for (arcade_setting_id id = 0; id < ARCADE_SETTING_COUNT; id++) {
            fn(id, arcade_settings_get(id), ctx);
        }
        return ARCADE_SETTING_COUNT;
    }
     ...
     * the same order `arcade schema` names them.
     */
    int seen = 0;
    for (arcade_setting_id id = 0; id < ARCADE_SETTING_COUNT; id++) {
...
int arcade_profile_save(int slot, const char *name) {
...
/*
 * Every value goes through arcade_settings_set(), so a profile written by an
...
int arcade_profile_load(int slot, bool *reboot, arcade_profile_progress_cb progress) {
    char leaving[ARCADE_PROFILE_NAME_LEN];
     ...
     */
    int rc = arcade_profile_name(current_slot, leaving, sizeof(leaving));
    if (rc < 0) {
        if (current_slot != ARCADE_PROFILE_DEFAULT_SLOT) {
            return rc;
        }
        snprintf(leaving, sizeof(leaving), "%s", ARCADE_PROFILE_DEFAULT_NAME);
    }
    ...
    for (uint16_t i = 0; i < scratch.entries; i++) {
        ...
        if (arcade_settings_get(id) == scratch.entry[i].value &&
            arcade_settings_is_stored(id)) {
            continue;
        }
        if (arcade_settings_set(id, scratch.entry[i].value) == 0) {
            moved++;
            if (reboot && !arcade_settings_describe(id)->live) {
                *reboot = true;
            }
        } else {
            LOG_WRN("Profile %d has no usable value for %s", slot,
                    arcade_settings_describe(id)->name);
        }
    }
...
int arcade_profile_rename(int slot, const char *name) {
    int rc = fetch(slot);
...
int arcade_profile_delete(int slot) {
    char key[24];
     ...
     */
    if (slot == ARCADE_PROFILE_DEFAULT_SLOT || slot == arcade_profile_current()) {
        return -EPERM;
    }
    slot_key(slot, key, sizeof(key));
...
void arcade_profile_stage_clear(void) {
...
int arcade_profile_stage(arcade_setting_id id, uint32_t value) {
    if (!hashes_usable()) {
        return -EEXIST;
    }
    if (!staging) {
    ...
    if (staged.entries >= ARCADE_SETTING_COUNT) {
        return -ENOSPC;
    }
    staged.entry[staged.entries].key = hashes[id];
 ...
 */
int arcade_profile_commit(int slot, const char *name) {
    if (arcade_profile_slot_valid(slot) && slot == arcade_profile_current()) {
        return -EBUSY;
    }
    return commit_staged(slot, name);
}
