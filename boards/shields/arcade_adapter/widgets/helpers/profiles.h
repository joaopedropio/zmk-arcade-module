/*
 * Arcade dongle - named sets of settings, kept on the dongle.
 *
...
#define ARCADE_PROFILE_NAME_LEN 24

/* how many profiles the dongle has room for */
#define ARCADE_PROFILE_SLOTS CONFIG_ARCADE_PROFILE_SLOTS

/* the one a dongle is flashed with, and the one it falls back to */
#define ARCADE_PROFILE_DEFAULT_SLOT 0
#define ARCADE_PROFILE_DEFAULT_NAME "Default"
...
bool arcade_profile_slot_valid(int slot);
 ...
 */
typedef void (*arcade_profile_cb)(arcade_setting_id id, uint32_t value, void *ctx);
 ...
 */
typedef void (*arcade_profile_progress_cb)(uint16_t done, uint16_t total);
