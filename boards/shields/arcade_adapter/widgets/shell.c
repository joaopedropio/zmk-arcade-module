/*
 * Arcade dongle - the settings shell.
 *
 * An `arcade` command on whatever shell backend the build has, so the dongle
 ...
 *
 * `arcade schema` is the same information with the prose taken out: one
 ...
 *
 * `arcade profile` keeps named sets of all of it on the dongle, and the dongle
...
static const char *kind_name(arcade_value_kind kind) {
    switch (kind) {
    case ARCADE_VALUE_ENUM:
        return "enum";
    case ARCADE_VALUE_COLOR:
        return "color";
    default:
        return "number";
    }
...
static void print_value(const struct shell *sh, arcade_setting_id id) {
    const arcade_setting *desc = arcade_settings_describe(id);
    char value[ARCADE_SETTING_VALUE_LEN];

    shell_print(sh, "%s %-24s %-14s %s", arcade_settings_is_stored(id) ? "*" : " ", desc->name,
                arcade_settings_format(id, arcade_settings_get(id), value, sizeof(value)),
                desc->live ? "live" : "next boot");
}

static void print_accepted(const struct shell *sh, arcade_setting_id id) {
    const arcade_setting *desc = arcade_settings_describe(id);
    ...
    if (desc->kind == ARCADE_VALUE_NUMBER) {
        shell_print(sh, "%s takes %u to %u", desc->name, desc->min, arcade_settings_max(id));
        return;
    }

    shell_fprintf(sh, SHELL_NORMAL, "%s takes", desc->name);
    for (uint32_t value = desc->min; value <= arcade_settings_max(id); value++) {
...
static int cmd_get(const struct shell *sh, size_t argc, char **argv) {
    if (argc == 2) {
        int id = arcade_settings_find(argv[1]);
        if (id < 0) {
...
static int cmd_schema(const struct shell *sh, size_t argc, char **argv) {
    ...
    for (arcade_setting_id id = 0; id < ARCADE_SETTING_COUNT; id++) {
        ...
        shell_fprintf(sh, SHELL_NORMAL, "%s\t%s\t%s\t%s\t%s\t%u\t%u\t", desc->name,
                      kind_name(desc->kind),
                      arcade_settings_format(id, arcade_settings_get(id), value, sizeof(value)),
                      arcade_settings_is_stored(id) ? "stored" : "default",
                      desc->live ? "live" : "boot", desc->min, arcade_settings_max(id));

        if (desc->kind != ARCADE_VALUE_ENUM) {
            shell_fprintf(sh, SHELL_NORMAL, "-\n");
            continue;
        }
        for (uint32_t v = desc->min; v <= arcade_settings_max(id); v++) {
...
static int cmd_set(const struct shell *sh, size_t argc, char **argv) {
    ...
    int64_t value = arcade_settings_parse(id, argv[2]);
    if (value < 0) {
    ...
    int rc = arcade_settings_set(id, (uint32_t)value);
    if (rc == -EINVAL) {
...
static int cmd_reset(const struct shell *sh, size_t argc, char **argv) {
    ...
    if (strcmp(argv[1], "all") == 0) {
        for (arcade_setting_id id = 0; id < ARCADE_SETTING_COUNT; id++) {
            if (arcade_settings_is_stored(id)) {
                arcade_settings_forget(id);
            }
        }
    ...
    int rc = arcade_settings_forget(id);
    if (rc) {
...
static int slot_arg(const struct shell *sh, const char *word) {
    ...
    if (*word == '\0' || *end != '\0' || !arcade_profile_slot_valid((int)slot)) {
        shell_error(sh, "there is no profile slot %s; they run 0 to %d", word,
                    ARCADE_PROFILE_SLOTS - 1);
        return -EINVAL;
    }
    return (int)slot;
}
...
static int cmd_profile_list(const struct shell *sh, size_t argc, char **argv) {
    ...
    for (int slot = 0; slot < ARCADE_PROFILE_SLOTS; slot++) {
        char name[ARCADE_PROFILE_NAME_LEN];
        int size = arcade_profile_name(slot, name, sizeof(name));
...
static int cmd_profile_current(const struct shell *sh, size_t argc, char **argv) {
    ...
    int slot = arcade_profile_current();
    char name[ARCADE_PROFILE_NAME_LEN];

    if (arcade_profile_name(slot, name, sizeof(name)) < 0) {
        snprintf(name, sizeof(name), "%s", ARCADE_PROFILE_DEFAULT_NAME);
    }
...
static void print_entry(arcade_setting_id id, uint32_t value, void *ctx) {
    const struct shell *sh = ctx;
    char text[ARCADE_SETTING_VALUE_LEN];

    shell_print(sh, "%s\t%s", arcade_settings_describe(id)->name,
                arcade_settings_format(id, value, text, sizeof(text)));
}
...
static int cmd_profile_show(const struct shell *sh, size_t argc, char **argv) {
    ...
    int rc = arcade_profile_read(slot, print_entry, (void *)sh);
    if (rc < 0) {
...
static int cmd_profile_save(const struct shell *sh, size_t argc, char **argv) {
    ...
    int rc = arcade_profile_save(slot, argv[2]);
    if (rc) {
    ...
    shell_print(sh, "saved %d settings to profile %d as \"%s\"; the dongle is on it now",
                ARCADE_SETTING_COUNT, slot, argv[2]);
    return 0;
}
...
static int cmd_profile_load(const struct shell *sh, size_t argc, char **argv) {
    ...
    bool reboot = false;
    int moved = arcade_profile_load(slot, &reboot, NULL);
    if (moved < 0) {
...
static int cmd_profile_rename(const struct shell *sh, size_t argc, char **argv) {
    ...
    int rc = arcade_profile_rename(slot, argv[2]);
    if (rc) {
...
static int cmd_profile_delete(const struct shell *sh, size_t argc, char **argv) {
    ...
    int rc = arcade_profile_delete(slot);
    if (rc == -EPERM) {
        shell_error(sh, "profile %d cannot be forgotten; %s", slot,
                    slot == ARCADE_PROFILE_DEFAULT_SLOT
...
static int cmd_profile_stage(const struct shell *sh, size_t argc, char **argv) {
    ...
    int rc = arcade_profile_stage(id, (uint32_t)value);
    if (rc) {
...
static int cmd_profile_commit(const struct shell *sh, size_t argc, char **argv) {
    ...
    int rc = arcade_profile_commit(slot, argv[2]);
    if (rc == -ENODATA) {
...
static void setting_names(size_t idx, struct shell_static_entry *entry) {
    entry->syntax = idx < ARCADE_SETTING_COUNT ? arcade_settings_describe(idx)->name : NULL;
    entry->handler = NULL;
    entry->subcmd = NULL;
    entry->help = NULL;
}
...
SHELL_CMD_REGISTER(arcade, &arcade_subcmds, "What the Arcade dongle remembers", NULL);
