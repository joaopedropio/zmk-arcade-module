 ...
 * the game and the dashboard, a hold past ARCADE_THEME_THRESHOLD moves to the
 * next profile, and a longer one mutes.
 *
 * The dashboard is a grid of slots - what goes in each one is Kconfig, see
 * ARCADE_INFO_SLOT_* - drawn straight to the panel by the widgets themselves.
...
void refresh_screen(void) {
    ...
    arcade_start(); /* the maze is repainted from the top on the next frame */
}

void print_menu(void) {
    if (get_dashboard_style() == DASHBOARD_STYLE_CABINET) {
        /*
         * The cabinet dashboard owns the whole panel and reads the widgets'
         * state back itself, so the slot widgets are stopped - only cabinet.c
         * draws while this is up.  Stopping them here and not only in
         * toggle_menu() is what makes `arcade set dashboard-style cabinet` safe
...
static void switch_profile(struct k_work *work) {
     ...
     */
    arcade_profile_load(slot, NULL, on_progress);
...
static void toggle_mute(void) { arcade_settings_toggle_mute(); }
