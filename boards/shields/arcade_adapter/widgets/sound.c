 ...
 *
 * The synth in game/arcade_sfx.c does the actual sound and knows nothing about
...
#include "game/arcade_sfx.h"
#include "helpers/settings.h"
#include "sound.h"

LOG_MODULE_DECLARE(arcade, LOG_LEVEL_INF);

#define AMP_NODE DT_NODELABEL(arcade_amp)

#if IS_ENABLED(CONFIG_ARCADE_SOUND) && DT_NODE_HAS_STATUS(AMP_NODE, okay)
 ...
 * wants in an atomic and wakes the thread.  ARC_TUNE_QUIET is "stop".
 */
#define ARC_TUNE_QUIET (ARC_TUNE_COUNT + 1)

static atomic_t pending = ATOMIC_INIT(ARC_TUNE_NONE);
...
static int write_block(void) {
    ...
    arc_sfx_render(mono, FRAMES_PER_BLOCK);
...
/* the pending request, applied to the synth; ARC_TUNE_QUIET stops it */
static void take_request(void) {
    atomic_val_t asked = atomic_set(&pending, ARC_TUNE_NONE);

    if (asked == ARC_TUNE_QUIET) {
        arc_sfx_stop();
    } else if (asked != ARC_TUNE_NONE) {
        arc_sfx_play((arc_tune_id)asked);
    }
}
 ...
 * screen was busy.
 */
K_THREAD_DEFINE(arcade_sound, 2048, sound_thread, NULL, NULL, NULL, K_PRIO_PREEMPT(3), 0, 0);
 ...
 * rather, not heard.
 */
static void request(arc_tune_id id) {
    if (!ready || arcade_settings_get_mute()) {
        return;
    }
...
void arcade_sound_init(void) {
    ...
    struct i2s_config cfg = {
        ...
        .frame_clk_freq = arcade_settings_get(ARCADE_SETTING_SAMPLE_RATE),
        .mem_slab = &sound_slab,
    ...
    arc_sfx_init(arcade_settings_get(ARCADE_SETTING_SAMPLE_RATE),
                (uint8_t)arcade_settings_get(ARCADE_SETTING_VOLUME),
                (uint16_t)arcade_settings_get(ARCADE_SETTING_BASS_FLOOR));
    ready = true;
...
void arcade_sound_quiet(void) {
    ...
    atomic_set(&pending, ARC_TUNE_QUIET);
    k_sem_give(&wake);
}

void arcade_sound_connected(bool connected) {
    if (!IS_ENABLED(CONFIG_ARCADE_SOUND_CONNECT)) {
        return;
    }
    request(connected ? ARC_TUNE_CONNECT : ARC_TUNE_DISCONNECT);
}

void arcade_sound_set_volume(uint8_t volume) { arc_sfx_set_volume(volume); }

void arcade_sound_set_bass_floor(uint16_t floor_hz) { arc_sfx_set_bass_floor(floor_hz); }

void arcade_sound_set_mute(bool muted) {
    if (muted) {
        arcade_sound_quiet(); /* mid-tune */
        return;
    }
    /*
...
void arcade_sound_init(void) {}
void arcade_sound_quiet(void) {}
void arcade_sound_connected(bool connected) { ARG_UNUSED(connected); }
void arcade_sound_set_mute(bool muted) { ARG_UNUSED(muted); }
void arcade_sound_set_volume(uint8_t volume) { ARG_UNUSED(volume); }
void arcade_sound_set_bass_floor(uint16_t floor_hz) { ARG_UNUSED(floor_hz); }
