 ...
 * stays for ARCADE_SPLASH_FRAMES ticks - long enough that LVGL's initial
 * flush of the empty screen cannot land on top of the game.  Then either the
 * game or the dashboard takes over, depending on ARCADE_DEFAULT_SCREEN, and
...
static void timer_splash(lv_timer_t *timer) {
    ...
    if (splash_count >= arcade_settings_get(ARCADE_SETTING_SPLASH_FRAMES)) {
        clean_up_splash();
...
lv_obj_t *zmk_display_status_screen(void) {
    ...
    lv_timer_create(timer_splash, arcade_settings_get(ARCADE_SETTING_SPLASH_INTERVAL),
                    NULL);
    if (get_slot_mode() == SLOT_MODE_2) {
        lv_timer_create(logo_animation_timer,
                        arcade_settings_get(ARCADE_SETTING_LOGO_INTERVAL), NULL);
    }
