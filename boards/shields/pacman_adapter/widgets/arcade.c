 ...
 * defines a chunky 5x7 arcade font and a round lamp of its own, and blits them
 ...
 * `pacman set` repaint this the same way it repaints the classic one, through
 * refresh_screen().
 *
 ...
 * listener then calls the matching arcade_refresh_*(), the same place it would
...
/* a fixed-width decimal, zero-padded, the way an arcade score reads */
...
void arcade_set_active(bool on) { active = on; }
