...
static void move_balls(fg_game *g) {
    for (int i = 0; i < 2; i++) {
        ...
        if (FG_PX(b->x) < -FG_BALL_R || FG_PX(b->x) > PM_PANEL + FG_BALL_R) {
            b->live = false;
        }
    }
...
static void clamp_x(fg_fighter *f) {
    int lo = FG_WALL * FG_SUB, hi = (PM_PANEL - FG_WALL) * FG_SUB;
...
static void stand(fg_game *g, int who) {
    ...
    f->x = (int16_t)((who == 0 ? FG_START_X : PM_PANEL - FG_START_X) * FG_SUB);
    f->h = 0;
    f->vy = 0;
    f->push = 0;
    f->air = 0;
