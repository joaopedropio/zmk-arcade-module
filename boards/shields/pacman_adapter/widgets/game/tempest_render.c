...
void tp_render_default_palette(tp_palette *p) {
    ...
    p->well = pm_rgb565(0x2040d0);
    p->rim = pm_rgb565(0x60a0ff);
    p->claw = pm_rgb565(0xffe030);
    p->shot = pm_rgb565(0xffffff);
    p->flipper = pm_rgb565(0xff3040);
    p->tanker = pm_rgb565(0x40e060);
    p->spiker = pm_rgb565(0x30e0e0);
    p->pulsar = pm_rgb565(0xff40e0);
    p->spike = pm_rgb565(0x9060ff);
    p->bolt = pm_rgb565(0xffa020);
 ...
 * hundred and twenty pixels of pm_put() saying no.
 */
...
static bool band_hits(int y, int h) { return !(y + h <= pm_by || y >= pm_by + pm_bh); }
...
static void span_box(const tp_shape *s, int a8, int b8, int d0, int d1, int pad, int *x, int *y,
    ...
    int x0 = PM_PANEL * 4, x1 = -PM_PANEL * 4, y0 = x0, y1 = x1;
    for (int i = 0; i < n; i++) {
        for (int k = 0; k < 2; k++) {
            int px, py;
...
static void draw_hud(const tp_game *g) {
    ...
    pm_text(TP_HUD_X, TP_HUD_Y, 2, pal.hud, buf);
    ...
    pm_text(TP_LEVEL_X, TP_LEVEL_Y, 1, pal.hud, buf);
}

static void draw_banner(const tp_game *g) {
    const char *word = tp_banner(g);

    if (word == 0 || !band_hits(TP_BANNER_Y - 4, PM_GLYPH_H * TP_BANNER_SCALE + 8)) {
        return;
    }
    int w = pm_text_w(word, TP_BANNER_SCALE);
    pm_fill((PM_PANEL - w) / 2 - 4, TP_BANNER_Y - 4, w + 8, PM_GLYPH_H * TP_BANNER_SCALE + 8,
            pal.site);
    pm_text((PM_PANEL - w) / 2, TP_BANNER_Y, TP_BANNER_SCALE, pal.hud, word);
}
...
static void repaint_text(const tp_game *g) {
    ...
    if (strcmp(now, prev_banner) != 0) {
        ...
        int was = pm_text_w(prev_banner, TP_BANNER_SCALE);
        int is = pm_text_w(now, TP_BANNER_SCALE);
        int w = (was > is ? was : is) + 10;
        memcpy(prev_banner, now, strlen(now) + 1);
        paint(g, (PM_PANEL - w) / 2, TP_BANNER_Y - 5, w, PM_GLYPH_H * TP_BANNER_SCALE + 10);
    }
}
