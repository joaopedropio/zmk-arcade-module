...
static void draw_rock(const ss_rock *rk) {
    ...
    for (int y = cy - r; y <= cy + r; y++) {
        if (y < pm_by || y >= pm_by + pm_bh) {
            continue;
        }
        for (int x = cx - r; x <= cx + r; x++) {
            if (x < pm_bx || x >= pm_bx + pm_bw) {
                continue;
            }
            int px = rock_px(rk, x - cx, y - cy);
            if (px != SS_RPX_OUT) {
                pm_put(x, y, px == SS_RPX_EDGE ? pal.rock_edge : pal.rock);
            }
        }
    }
}
...
static void draw_shot(const ss_shot *s) {
    ...
    pm_fill(hx, hy, 2, 2, pal.bullet);
    for (int k = 1; k <= SS_TAIL; k++) {
        pm_put(hx - (int)s->vx * k / (4 * SS_SUB), hy - (int)s->vy * k / (4 * SS_SUB),
            pal.bullet);
    }
}
...
static void draw_drop(const ss_game *g) {
    ...
    for (int y = cy - r; y <= cy + r; y++) {
        for (int x = cx - r; x <= cx + r; x++) {
            ...
            if (d <= r && d > r - 2) {
                pm_put(x, y, pal.power);
            }
        }
    }
    char word[2] = {POWER_LETTER[g->drop.kind], '\0'};
    pm_text(cx - 2, cy - 3, 1, pal.power, word);
}
    ...
    for (int y = y0; y <= y1; y++) {
        for (int x = x0; x <= x1; x++) {
            if (g->ship.thrusting && in_tri(x, y, flame)) {
                pm_put(x, y, pal.thruster);
            }
            if (!in_tri(x, y, hull) || in_tri(x, y, notch)) {
                continue;
            }
            pm_put(x, y, in_tri(x, y, pit) ? pal.trim : pal.ship);
        }
    }
...
static void draw_life(int x, int y) {
    for (int j = 0; j < 5; j++) {
        ...
        pm_fill(x + (7 - w) / 2, y + j, w, 1, pal.ship);
    }
}
...
static void draw_hud(const ss_game *g) {
    ...
    pm_text(SS_HUD_X, SS_HUD_Y, 2, pal.hud, buf);
    ...
    if (power != NULL) {
        pm_text(PM_PANEL - SS_HUD_X - pm_text_w(power, 1), SS_FOOT_Y, 1, pal.power, power);
    }
}
...
static void draw_banner(const ss_game *g) {
    ...
    pm_text((PM_PANEL - pm_text_w(word, SS_BANNER_SCALE)) / 2, SS_BANNER_Y, SS_BANNER_SCALE,
              pal.hud, word);
}
...
static void paint_band(const ss_game *g, int x0, int y0, int w, int h) {
    ...
    pm_fill(x0, y0, w, h, pal.space);

    for (int i = 0; i < SS_STARS; i++) {
        if (ss_star_lit(g, i)) {
            pm_put(g->stars[i].x, g->stars[i].y, pal.star);
        }
    }
...
static void repaint_text(const ss_game *g) {
    ...
    if (strcmp(buf, prev_hud) != 0) {
        ...
        paint(g, 0, 0, PM_PANEL, SS_HUD_Y + 2 * PM_GLYPH_H + 2);
    }

    foot_key(g, buf);
    if (strcmp(buf, prev_foot) != 0) {
        memcpy(prev_foot, buf, strlen(buf) + 1);
        paint(g, 0, SS_FOOT_Y - 1, PM_PANEL, PM_GLYPH_H + 2);
    }
    ...
    if (strcmp(now, prev_banner) != 0) {
        ...
        int was = pm_text_w(prev_banner, SS_BANNER_SCALE);
        int is = pm_text_w(now, SS_BANNER_SCALE);
        int w = (was > is ? was : is) + 2;
        memcpy(prev_banner, now, strlen(now) + 1);
        paint(g, (PM_PANEL - w) / 2, SS_BANNER_Y - 1, w, PM_GLYPH_H * SS_BANNER_SCALE + 2);
    }
}
