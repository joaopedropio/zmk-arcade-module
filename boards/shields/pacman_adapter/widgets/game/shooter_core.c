...
static void spawn_rock(ss_game *g) {
    ...
    int along = range(g, 0, PM_PANEL - 1);
    int x, y;
    ...
    int aim_x = range(g, PM_PANEL / 4, 3 * PM_PANEL / 4);
    int aim_y = range(g, PM_PANEL / 4, 3 * PM_PANEL / 4);
...
static bool shot_lands(const ss_game *g, uint8_t heading) {
    ...
    for (int t = 2; t <= SS_SHOT_LIFE; t += 3) {
        ...
        if (bx < 0 || bx >= PM_PANEL || by < 0 || by >= PM_PANEL) {
...
static void pilot(ss_game *g) {
    ...
    int cx = px - PM_PANEL / 2, cy = py - PM_PANEL / 2;
    bool near_wall = px < SS_EDGE || px > PM_PANEL - SS_EDGE || py < SS_EDGE ||
                     py > PM_PANEL - SS_EDGE;
...
static void move_ship(ss_game *g) {
    ...
    int hi = (PM_PANEL - 1 - SS_HULL_R) * SS_SUB;
    if (g->ship.x < lo) {
        g->ship.x = (int16_t)lo;
        g->ship.vx = 0;
    }
...
static bool gone(int x, int y, int margin) {
    return x < -margin * SS_SUB || x > (PM_PANEL + margin) * SS_SUB ||
           y < -margin * SS_SUB || y > (PM_PANEL + margin) * SS_SUB;
}
...
static void respawn(ss_game *g) {
    g->ship.x = (int16_t)(PM_PANEL * SS_SUB / 2);
    g->ship.y = (int16_t)(PM_PANEL * SS_SUB / 2);
    g->ship.vx = 0;
    g->ship.vy = 0;
