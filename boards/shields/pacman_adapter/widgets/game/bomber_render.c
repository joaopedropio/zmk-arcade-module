...
void bb_render_default_palette(bb_palette *p) {
    p->floor = pm_rgb565(0x1c6b2a);
    p->solid = pm_rgb565(0x9aa2ad);
    p->brick = pm_rgb565(0xb5561e);
    p->brick_edge = pm_rgb565(0x6e2f10);
    p->bomb = pm_rgb565(0x14161c);
    p->flame = pm_rgb565(0xff6a00);
    p->flame_hot = pm_rgb565(0xffe066);
    p->bomber = pm_rgb565(0xf2f2f2);
    p->bomber_trim = pm_rgb565(0x1e73ff);
    p->foe = pm_rgb565(0xe23b6d);
    p->foe_eye = pm_rgb565(0xffffff);
    p->pickup = pm_rgb565(0x00d2ff);
    p->door = pm_rgb565(0xffd400);
...
static void draw_floor(int x, int y, int r, int c) {
    ...
    pm_fill(x, y, BB_CELL, BB_CELL, base);
}

static void draw_solid(int x, int y, bool flash) {
    uint16_t face = flash ? pal.flame_hot : pal.solid;

    pm_fill(x, y, BB_CELL, BB_CELL, face);
    pm_fill(x, y, BB_CELL, 2, shade(face, 5, 4));
    pm_fill(x, y, 2, BB_CELL, shade(face, 5, 4));
    pm_fill(x, y + BB_CELL - 2, BB_CELL, 2, shade(face, 5, 8));
    pm_fill(x + BB_CELL - 2, y, 2, BB_CELL, shade(face, 5, 8));
}
...
static void draw_brick(int x, int y, bool flash) {
    ...
    pm_fill(x, y, BB_CELL, BB_CELL, face);
    pm_fill(x, y, BB_CELL, 1, pal.brick_edge);
    pm_fill(x, y + 8, BB_CELL, 1, pal.brick_edge);
    pm_fill(x + 4, y + 1, 1, 7, pal.brick_edge);
    pm_fill(x + 12, y + 1, 1, 7, pal.brick_edge);
    pm_fill(x + 8, y + 9, 1, 7, pal.brick_edge);
    pm_fill(x, y + 9, 1, 7, pal.brick_edge);
}
...
static void draw_item(int x, int y, int kind, bool usable) {
    if (kind == BB_I_DOOR) {
        ...
        pm_fill(x + 2, y + 3, 12, 13, face);
        pm_fill(x + 4, y + 6, 8, 10, shade(face, 1, 3));
        pm_fill(x + 5, y + 5, 6, 2, shade(face, 1, 3));
        return;
    }

    pm_fill(x + 2, y + 2, 12, 12, pal.pickup);
    pm_fill(x + 2, y + 2, 12, 1, shade(pal.pickup, 5, 4));
    pm_fill(x + 2, y + 13, 12, 1, shade(pal.pickup, 1, 2));

    uint16_t ink = shade(pal.pickup, 1, 5);
    switch (kind) {
    case BB_I_BOMB: /* one more of them at a time: a bomb */
        pm_fill(x + 5, y + 7, 6, 5, ink);
        pm_fill(x + 6, y + 6, 4, 1, ink);
        pm_fill(x + 9, y + 4, 2, 2, ink);
        break;
    case BB_I_FLAME: /* one more cell of reach: a flame */
        pm_fill(x + 7, y + 4, 2, 8, ink);
        pm_fill(x + 5, y + 6, 6, 2, ink);
        pm_fill(x + 4, y + 10, 8, 2, ink);
        break;
    default: /* a pixel a frame quicker: a pair of chevrons */
        for (int i = 0; i < 4; i++) {
            pm_fill(x + 4 + i, y + 4 + i, 1, 2, ink);
            pm_fill(x + 4 + i, y + 10 - i, 1, 2, ink);
            pm_fill(x + 8 + i, y + 4 + i, 1, 2, ink);
            pm_fill(x + 8 + i, y + 10 - i, 1, 2, ink);
        }
        break;
    }
}
...
static void draw_bomb(int x, int y, int phase) {
    ...
    for (int j = -r; j <= r; j++) {
        for (int i = -r; i <= r; i++) {
            if (i * i + j * j <= r * r) {
                pm_put(x + 8 + i, y + 8 + j, pal.bomb);
            }
        }
    }
    /* the highlight that stops a dark disc reading as a hole in the floor */
    pm_fill(x + 5, y + 5, 2, 2, shade(pal.bomb, 9, 4));
    pm_fill(x + 9, y + 2, 2, 2, pal.flame_hot);
    pm_fill(x + 8, y + 3, 1, 2, shade(pal.flame_hot, 1, 2));
}
...
static void draw_fire(int x, int y, int life, int mask) {
    ...
    pm_fill(x + x0, y + 8 - th, x1 - x0, th * 2, pal.flame);
    pm_fill(x + 8 - th, y + y0, th * 2, y1 - y0, pal.flame);

    int hot = th - 2;
    if (hot > 0) {
        pm_fill(x + x0, y + 8 - hot, x1 - x0, hot * 2, pal.flame_hot);
        pm_fill(x + 8 - hot, y + y0, hot * 2, y1 - y0, pal.flame_hot);
    }
}
...
static void draw_bomber(int x, int y, int dir, int stride) {
    ...
    pm_fill(x + 3, y + 12, 4, 3, stride ? pal.bomber : shade(pal.bomber, 1, 2));
    pm_fill(x + 9, y + 12, 4, 3, stride ? shade(pal.bomber, 1, 2) : pal.bomber);

    switch (dir) {
    case BB_UP:
        pm_fill(x + 4, y + 2, 8, 3, pal.bomber_trim);
        break;
    case BB_DOWN:
        pm_fill(x + 4, y + 8, 8, 3, pal.bomber_trim);
        pm_fill(x + 5, y + 5, 2, 2, pal.bomber_trim);
        pm_fill(x + 9, y + 5, 2, 2, pal.bomber_trim);
        break;
    case BB_LEFT:
        pm_fill(x + 2, y + 5, 5, 4, pal.bomber_trim);
        break;
    default:
        pm_fill(x + 9, y + 5, 5, 4, pal.bomber_trim);
        break;
    }
}
...
static void draw_foe(int x, int y, int kind, int stride) {
    if (kind == BB_F_DRIFT) {
        ...
        pm_fill(x + 2 + stride, y + 13, 3, 2, shade(pal.foe, 1, 2));
        pm_fill(x + 11 - stride, y + 13, 3, 2, shade(pal.foe, 1, 2));
    } else {
        for (int j = 0; j < 14; j++) {
            int half = j < 7 ? j : 13 - j;
            pm_fill(x + 7 - half, y + 1 + j, half * 2 + 2, 1, pal.foe);
        }
        /* the spikes, which is what a diamond needs to stop reading as a kite */
        pm_fill(x + 1, y + 7 - stride, 2, 2, pal.foe);
        pm_fill(x + 13, y + 7 + stride, 2, 2, pal.foe);
    }
    pm_fill(x + 5, y + 6, 2, 3, pal.foe_eye);
    pm_fill(x + 9, y + 6, 2, 3, pal.foe_eye);
}
...
static void draw_mark(int x, int y, int which) {
    ...
    case 0: /* lives: the bomber's helmet */
        ...
        pm_fill(x + 1, y + 3, 7, 2, pal.bomber_trim);
        break;
    case 1: /* bombs at once, rimmed: the bomb's own colour is nearly the
        ...
        pm_fill(x + 5, y, 2, 2, pal.flame_hot);
        break;
    default: /* how far an arm reaches */
        pm_fill(x + 3, y, 2, 9, pal.flame);
        pm_fill(x + 1, y + 3, 6, 2, pal.flame);
        pm_fill(x + 3, y + 3, 2, 2, pal.flame_hot);
        break;
    }
}
...
static void draw_hud(const bb_game *g) {
    ...
    pm_text(BB_HUD_X, BB_HUD_Y, 2, pal.hud, buf);

    /* clear of the digits' baseline, or the bar reads as an underline of the
     * score rather than as the thing running out */
    int bar = clock_bar(g), by = BB_HUD_Y + 2 * PM_GLYPH_H + 4;
    pm_fill(BB_HUD_X, by, 70, 3, shade(pal.hud, 1, 4));
    if (bar > 0) {
        pm_fill(BB_HUD_X, by, bar, 3, pal.hud);
    }
    ...
    for (int i = 0; i < 3; i++) {
        ...
        pm_digits(tally[i], 1, buf);
        pm_text(x + BB_ICON + 2, BB_HUD_Y + 2, 2, pal.hud, buf);
    }
}
...
static void draw_banner(const bb_game *g) {
    ...
    if (word != NULL) {
        pm_text((PM_PANEL - pm_text_w(word, BB_BANNER_SCALE)) / 2, BB_BANNER_Y,
...
static void paint_band(const bb_game *g, int x0, int y0, int w, int h) {
    ...
    pm_fill(x0, y0, w, h, shade(pal.solid, 1, 3));
...
static void repaint_text(const bb_game *g) {
    ...
    if (strcmp(now, prev_banner) != 0) {
        ...
        int was = pm_text_w(prev_banner, BB_BANNER_SCALE);
        int is = pm_text_w(now, BB_BANNER_SCALE);
        int w = (was > is ? was : is) + 2;
        memcpy(prev_banner, now, strlen(now) + 1);
        paint(g, (PM_PANEL - w) / 2, BB_BANNER_Y - 1, w, PM_GLYPH_H * BB_BANNER_SCALE + 2);
    }
}
