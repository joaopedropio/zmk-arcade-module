...
static int pacman_settings_set(const char *key, size_t len_rd, settings_read_cb read_cb, void *cb_arg) {
...
SETTINGS_STATIC_HANDLER_DEFINE(pacman_settings_handler, "pacman", NULL, pacman_settings_set, NULL, NULL);

int pacman_settings_save() {
    int rc = settings_save_one("pacman/settings", &current_settings, sizeof(current_settings));
    if (rc) {
...
int pacman_settings_save_mute(bool is_muted) {
    current_settings.mute = is_muted;
    return pacman_settings_save();
}

int pacman_settings_toggle_mute(void) {
    return pacman_settings_save_mute(!current_settings.mute);
}
