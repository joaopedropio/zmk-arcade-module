...
module that turns the dongle's 240x240 ST7789V panel into one of three games.
Nobody drives any of them.
...
**The game** owns the whole panel, and is the maze, the shooter or the
crossing depending on `game`. The shooter puts its score across the top, the
lives it has left beside it as small ships, and whatever pickup is running
along the bottom; the crossing does the same with small frogs, and puts the
...
| `CONFIG_PACMAN_DEFAULT_GAME` | `pacman` | Which game the game screen plays: `pacman`, `shooter` or `frogger`. |
...
| | Pac-Man | Space Shooter | Crossing |
|---|---|---|---|
| flash | 7.4 KB | 7.7 KB | 7.2 KB |
| RAM | 671 bytes | 916 bytes | 998 bytes |
| SPI traffic | ~3800 pixels/frame ≈ 7.4 KB/frame, about 220 KB/s at 30 fps | ~4000 pixels/frame ≈ 7.8 KB/frame, about 235 KB/s | ~8900 pixels/frame ≈ 17 KB/frame, about 260 KB/s at 15 fps |
...
the pixels are what the simulator counts over a long soak. On top of all three
sits the 10.3 KB band a rectangle is staged in before it goes down the SPI bus,
and the 5x7 font and stamping calls the two sprite renderers share — both in
`game/panel.h` rather than in any one renderer, because only one game is ever
running and a buffer each would be fifteen kilobytes spent on the games nobody
is watching. Even the dearest of them is an eighth of what a 20 MHz link
carries.
...
and the crossing were therefore never given any. What is left is the one thing the dongle knows that you cannot see:
...
Every game core and renderer is plain C with no Zephyr or LVGL dependencies,
so they build and run on a host. The simulator blits into a 240x240 buffer,
checks the invariants every frame (nobody inside a wall, nobody standing on
water, nobody off the panel, and the incremental redraw always matching a full
...
/tmp/pacman-sim frogger 3000
```

The game name goes in front and may be left out, in which case it is the maze.
Each of them prints what its own balance turns on — mazes cleared against
deaths, meteors destroyed against restarts, bays filled against what killed the
frog — so a change to any of them is judged over a long soak rather than over
ten seconds of watching. Building it with `-DFR_TRACE` makes the crossing name
every death and where it happened.
...
and `docs/frogger.gif` are the same for the other two.
...
tools/sim/                      host simulator for all three games
...
by João Pedro. Pac-Man is © Bandai Namco, Frogger is © Konami, and Space
